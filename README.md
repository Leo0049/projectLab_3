...
| 限流（分鐘） | 5 | 超過分鐘配額回正確錯誤並留痕 |
| 限流後端接線 | 5 | 確認實際生效的實作；設定錯誤時啟動失敗 |
| 啟動期檢查 | 5 | 模板缺 `:__tenant`、工具含 tenant 參數時啟動失敗 |
| 每日配額 | 4 | 分鐘配額還很寬鬆時，日配額仍會擋下 |
| 回傳筆數上限 | 4 | 超過 200 列會截斷並明說，不靜默丟資料 |
| MCP Resource | 3 | 資料字典已註冊且不含任何客戶資料 |
| 工具註冊 | 2 | 8 個工具確實註冊且治理鏈生效 |
| 審批台呈現（單元） | 15 | before/after 讀得出來；會失敗的請求當場標示；到期時間換算成當地時間 |
...
除了 237 個自動化測試，另外做過一次**實機排練**：用打包好的 jar，接真的
...
**這次排練抓到三個單元測試沒抓到的問題**，都已修正並補上測試：

1. demo 種子把 50,000 筆訂單全放在租戶 7——等於對著一個空租戶示範隔離。
2. 待審批訊息裡的 `approvalId=%s` 沒被代入。Java 的 `a + b + c.formatted(x)`
   只把 `formatted` 綁在 `c` 上，模型讀到的是字面的 `%s`。
3. 審批頁把 preview 直接印成 JSON。規格書 §14 要求「顯示 before/after preview」，
   而 `{"after":100,"delta":-20,"before":120,...}` 是**包含**了 before/after，
   不是顯示。它同時讓那筆刻意失敗的請求顯示成 `"after": -999984`，看起來像系統算錯，
   而不是「這張單子核准後會被拒絕」。已改成一行 `珍珠奶茶 120 → 100` 加上具體警告，
   原始 JSON 收在可展開區塊裡（審批者拿的是未遮罩真值，摘要不能等於隱藏），
   到期時間也從 `2026-08-14T13:59:27.980013Z` 改成 `27 分鐘後到期（22:55）`。

第 3 項是三個裡最容易被當成「美觀問題」而略過的。它不是：審批機制的價值完全
建立在審批者真的看懂自己核准了什麼，看不懂的審批就是橡皮圖章。

---

## 完整性稽核發現的問題

專案完成後做了一次「有主張 vs 有測試」的比對，補到 196 個測試。過程中找到三個
**共同特徵是靜默失效**的缺陷——沒有例外、沒有 log，既有測試也全部照過：

| # | 問題 | 為什麼沒被發現 | 修法 |
|---|---|---|---|
| 1 | **`RedisRateLimiter` 從來沒有被建立過**，正式環境也一樣 | 後端選擇用了 `@ConditionalOnMissingBean`，但它在 user configuration 中會**早於** autoconfiguration 註冊 Redis 就被評估，所以 in-memory 版永遠勝出。多實例部署等於把配額乘上實例數 | 改成由設定明確指定後端；要 Redis 卻沒有 Redis 就**啟動失敗**，而不是默默降級 |
| 2 | `MaskingEngine` 遇到非 public 的 record 會拋例外 | 它對 canonical constructor 呼叫了 `setAccessible`，卻沒有對 component accessor 呼叫。正式 DTO 剛好都是 public，所以永遠沒踩到 | accessor 一併 `setAccessible` |
| 3 | 三個營收模板有兩個**從來沒執行過** | 工具測試的固定參數永遠是 `groupBy=STORE`，`sales.summary_by_day` 與 `sales.summary_by_product` 是未執行過的 SQL | 新增模板覆蓋測試：**未列入參數表的模板會讓建置失敗** |

另外兩個較小的問題：`inventory.single` 與 `singleProduct()` 是死碼（已移除）；
測試種子的 `order_items` 加總不等於訂單金額，導致同一期間「依店家」與「依品項」
的營收會對不起來（已修正，並加上一條測試釘住三種分組總額必須一致）。

第 1 項對面試特別有價值：它正是本專案在講的那種問題——**安全機制看起來有裝，
但其實沒有生效，而且不會有人知道**。
